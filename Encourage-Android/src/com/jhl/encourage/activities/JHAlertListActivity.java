...
public class JHAlertListActivity extends Activity {
    ...
    private void invokeAlertsApi(String start) {
    	...
    	service.getAlertDetails("getUserAlertsDetails", JHAppStateVariables.getLoginTocken(),JHUtility.getDateTime(), JHUtility.getTimeZoneString(), start, 
    			...
    			public void success(SpocResponse spocResponse, retrofit.client.Response arg1) {
    				...
    				for (SpocObject spocObject : responseList) {
    					if (spocObject.getResultTypeCode()
							...
							if (map.containsKey("lastcount")) {
								...
								if (map.get(JHConstants.NOT_XML_KEY_ALERT_TYPE).contains("Map")) {
									alert.setContenType(JHConstants.NOT_XML_KEY_ALERT_TYPE_MAP);
								}else if (map.get(JHConstants.NOT_XML_KEY_ALERT_TYPE).contains("Link")) {
									alert.setContenType(JHConstants.NOT_XML_KEY_ALERT_TYPE_LINK);
								}else if (map.get(JHConstants.NOT_XML_KEY_ALERT_TYPE).contains("Note")) {
									alert.setContenType(JHConstants.NOT_XML_KEY_ALERT_TYPE_NOTE);
								}else if (map.get(JHConstants.NOT_XML_KEY_ALERT_TYPE).contains("Image")) {
									alert.setContenType(JHConstants.NOT_XML_KEY_ALERT_TYPE_IMAGE);
								}
								
								alert.setId(map.get(JHConstants.NOT_XML_KEY_ALERT_KEY));
								alert.setNotificationType(map.get(JHConstants.NOT_XML_KEY_TYPE));
								alert.setAuthorName(map.get(JHConstants.NOT_XML_KEY_AUTHOR_NAME));
								alert.setContenType(map.get(JHConstants.NOT_XML_KEY_CONTENT_TYPE));
								alert.setDateTime(map.get(JHConstants.NOT_XML_KEY_DATE_TIME));
								alert.setDateTimeDiff(map.get(JHConstants.NOT_XML_KEY_DATE_TIME_DIFF));
								alert.setDetails(map.get(JHConstants.NOT_XML_KEY_DETAILS));
								alert.setNotificationType(map.get(JHConstants.NOT_XML_KEY_TYPE));
								alert.setReadStatus(map.get(JHConstants.NOT_XML_KEY_READ_STATUS));
								alert.setTitle(map.get(JHConstants.NOT_XML_KEY_TITLE));
								
								alert.setAlertType(map.get(JHConstants.NOT_XML_KEY_ALERT_TYPE));
								
								String url = map.get(JHConstants.NOT_XML_KEY_ALERT_LINK_KEY);
								String address = map.get(JHConstants.NOT_XML_KEY_ALERT_MAP_KEY);
								String imageName = map.get(JHConstants.NOT_XML_KEY_ALERT_IMAGE_KEY);
								String authorProfilePicName = map.get("author_profilepic_name");
